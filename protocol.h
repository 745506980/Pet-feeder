...
typedef struct raw{
	...
	unsigned char week;  			//前七位表示周一到周日 例如周日0x01
	unsigned char hour;  	// 按照小时分钟格式，高8位代表时刻，第八位代表分钟 
	unsigned char minute;			//	例如18:50      0x1232
	unsigned char weight;			// 喂食份数 例如12       0x0c
	unsigned char swch;			// 开关 00表示关闭 0x01表示开启
...
extern Feed_plan_buf feed_plan_buf;

extern Feed_Plan feed_plan[10];    //最多10个喂食计划

extern unsigned short feed_num;	// 计划的个数
...
typedef struct  //喂食结果上报
{
	unsigned long manual_feed;		//手动喂食 value
	unsigned long feed_report; 	// 喂食结果上报；  value
	unsigned char quick_feed; 		//快速喂食   bool
	unsigned char slow_feed;		//慢速喂食  bool 
	unsigned char feed_check;  		//出粮校准  bool 
	unsigned char light;       		//小夜灯  bool 
	unsigned long surplus_grain; 	//粮桶余粮
	unsigned long weight;			//余粮重量	
	unsigned long voice_times; 	//语音播报次数	 
	unsigned char feed_state;		// 喂食状态 
	unsigned char cover_state;		//喂食盖状态 
	unsigned char charge_state;		//充电状态 
	unsigned char battery_percentage;	// 电池电量 
	unsigned char export_state;		//出粮校准 	
}Feed_status;


extern Feed_status feed_status;
/******************************************************************************
                           用户相关信息配置
******************************************************************************/
/******************************************************************************
                       1:修改产品信息
******************************************************************************/
#define PRODUCT_KEY "x6lup6s6y5zif6mr"    //开发平台创建产品后生成的16位字符产品唯一标识
...
//#define WIFI_UART_RECV_BUF_LMT          128
#define WIFIR_UART_SEND_BUF_LMT         64              //根据用户DP数据大小量定,必须大于48
/******************************************************************************
                        4:定义模块工作方式
模块自处理:
...
#define         WIFI_CONTROL_SELF_MODE                       //wifi自处理按键及LED指示灯;如为MCU外界按键/LED指示灯请关闭该宏
  ...
  #define     WF_STATE_KEY            8                    //wifi模块状态指示按键，请根据实际GPIO管脚设置  SW5
