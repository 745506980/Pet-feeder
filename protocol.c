...
//#define SUPPORT_GREEN_TIME   //打开时间控制
...
void uart_transmit_output(unsigned char value)
{
	...
	int ret = write(fd_wifi_uart, &value, 1);
	if (-1 == ret)
	{
		perror("write");
		return;
	}
//	printf("ret:%d  tx:0x%x\n", ret, value);
/*
    //Example:
...
void init(void)
{
	feed_status.feed_check = TRUE;
	feed_status.feed_report = 0;
	feed_status.light = FALSE;
	feed_status.manual_feed = 0;
	feed_status.quick_feed = FALSE;
	feed_status.slow_feed = FALSE;
	feed_status.weight = 0;
	feed_status.surplus_grain = 0;
	feed_status.feed_state = standby;
	feed_status.cover_state = off;
	feed_status.charge_state = FALSE;
	feed_status.battery_percentage = 100;
	feed_status.export_state = TRUE;
}
...
//    #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    ...
    printf("all_data_update\n");
	mcu_dp_raw_update(DPID_MEAL_PLAN,feed_plan_buf.buf, feed_plan_buf.length); //RAW型数据上报;
    mcu_dp_bool_update(DPID_QUICK_FEED, feed_status.quick_feed); //BOOL型数据上报;
    mcu_dp_value_update(DPID_MANUAL_FEED,feed_status.manual_feed); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_FEED_STATE, feed_status.feed_state); //枚举型数据上报;
    mcu_dp_value_update(DPID_FEED_REPORT,feed_status.feed_report); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_SWITCH,0); //BOOL型数据上报;
	mcu_dp_enum_update(DPID_UNIT,cup);
    mcu_dp_bool_update(DPID_SLOW_FEED,feed_status.slow_feed); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_EXPORT_CALIBRATE,feed_status.feed_check); //BOOL型数据上报;
//    mcu_dp_value_update(DPID_FEED_REPORT,feed_status.feed_report); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_LIGHT,feed_status.light); //BOOL型数据上报;
//    mcu_dp_bool_update(DPID_SWITCH,当前开关); //BOOL型数据上报;

//	mcu_dp_bool_update(DPID_FACTORY_RESET,当前恢复出厂); //BOOL型数据上报;
	mcu_dp_enum_update(DPID_EXPORT_STATE, feed_status.export_state); //枚举型数据上报;
	mcu_dp_value_update(DPID_BATTERY_PERCENTAGE,feed_status.battery_percentage); //VALUE型数据上报;  //电池电量
	mcu_dp_bool_update(DPID_CHARGE_STATE,feed_status.charge_state); //BOOL型数据上报;	
	mcu_dp_enum_update(DPID_COVER_STATE, feed_status.cover_state); //枚举型数据上报;
//	mcu_dp_fault_update(DPID_FAULT,当前故障告警); //故障型数据上报;
//	mcu_dp_value_update(DPID_FEED_REPORT,当前喂食结果上报); //VALUE型数据上报;

	mcu_dp_value_update(DPID_SURPLUS_GRAIN,feed_status.surplus_grain); //VALUE型数据上报;
	mcu_dp_value_update(DPID_WEIGHT,feed_status.weight); //VALUE型数据上报;
	mcu_dp_value_update(DPID_VOICE_TIMES,feed_status.voice_times); //VALUE型数据上报;
}
...
Feed_Plan feed_plan[10];
Feed_plan_buf feed_plan_buf; 
unsigned short feed_num = 0;
void get_week(const unsigned char value, unsigned short i)
{


	bzero(&feed_plan[i], sizeof(Feed_Plan));
	if (0x01 == (value & 0x01))   //周日
		feed_plan[i].weekday[0] = 1;
	if (0x02 == (value & 0x02))
		feed_plan[i].weekday[6] = 1;
	if (0x04 == (value & 0x04))
		feed_plan[i].weekday[5] = 1;
	if (0x08 == (value & 0x08))
		feed_plan[i].weekday[4] = 1;	
	if (0x10 == (value & 0x10))
		feed_plan[i].weekday[3] = 1;
	if (0x20 == (value & 0x20))
		feed_plan[i].weekday[2] = 1;	
	if (0x40 == (value & 0x40))  // 周一
		feed_plan[i].weekday[1]= 1;
...
static unsigned char dp_download_meal_plan_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
	...
	{
		printf("喂食计划----%d\n", i);
		printf("Monday		: %d\n", feed_plan[i].weekday[0]);
		printf("Tuesday		: %d\n", feed_plan[i].weekday[1]);
		printf("Wednesday	: %d\n", feed_plan[i].weekday[2]);
		printf("Thursday	: %d\n", feed_plan[i].weekday[3]);
		printf("Friday		: %d\n", feed_plan[i].weekday[4]);
		printf("Satureday	: %d\n", feed_plan[i].weekday[5]);
		printf("Sunday		: %d\n", feed_plan[i].weekday[6]);
		printf("hour		: %d\n", feed_plan[i].hour);
		printf("minute		: %d\n", feed_plan[i].minute);
		printf("weight		: %d\n", feed_plan[i].weight);
		printf("swch		: %d\n", feed_plan[i].swch);
...
static unsigned char dp_download_quick_feed_handle(const unsigned char value[], unsigned short length)
{
    ...
    if(quick_feed == 0) {
        //开关关
       printf("no quick_feed\n");
	   feed_status.quick_feed = 0;
...
static unsigned char dp_download_slow_feed_handle(const unsigned char value[], unsigned short length)
{
    ...
    unsigned char ret;
    //0:关/1:开
    unsigned char slow_feedi;    
    slow_feedi = mcu_get_dp_download_bool(value,length);
    if(slow_feedi == 0) {
        //开关关
		feed_status.slow_feed = FALSE;
	}else {
        //开关开
		printf("slow feed!\n");
//		motor_on('R', slow_feed * 10, 3);
		feed_status.slow_feed = TRUE;
	}	
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_SLOW_FEED,slow_feedi);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
static unsigned char dp_download_light_handle(const unsigned char value[], unsigned short length)
{
    ...
    if(light == 0) {
        ...
        //开关开
		ioctl(led_fd, 1, 0);  //第二个参数代表命令0关 1 开, 第三个是哪一个led
	}
 ...
 * @note   MCU需要自行实现该功能
 */
void mcu_get_greentime(unsigned char time_[])
{
  //  #error "请自行完成相关代码,并删除该行"
    ...
    */
    ...
    if(time_[0] == 1) {
		...
		stpr.tm_hour = time_[4] + 8;   // 格林威治时区比北京时间多了8小时
		if (stpr.tm_hour > 24)
		{
			stpr.tm_hour = stpr.tm_hour / 24;
			stpr.tm_mday = time_[3] + 1;
		}
		else{
...
void get_wifi_status(unsigned char result)
{
  //#error "请自行完成获取 WIFI 状态结果代码,并删除该行"
	// 如果为链接上云端，则一直检测wifi连接状态
	unsigned char command[7]; 
    switch(result) {
        case 0:
            //wifi工作状态1
		//	sleep(2);
		//	mcu_get_wifi_connect_status(); // 获取wifi连接状态
			printf("smartconfig 配置状态\n");			
		break;
...
void set_speaker_voice_result(unsigned char result)
{
  //  #error "请自行实现speaker音量设置结果处理代码,完成后请删除该行"
	...
	printf("video set is success  speaker is :%d\n", result);
}
