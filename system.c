...
volatile unsigned char stop_update_flag;                                                 //ENABLE:停止一切数据上传  DISABLE:恢复一切数据上传
...
volatile unsigned char stream_status;                                                             //流服务发包返回状态
volatile unsigned char maps_stream_status;                                                        //多地图流服务发包返回状态
#endif
*/
/**
 * @brief  写wifi_uart字节
...
static unsigned char data_point_handle(const unsigned char value[])
{
    ...
    if(dp_type != download_cmd[index].dp_type) {
        //错误提示
		printf("data_point_handle is error!\n");
        return FALSE;
    }else {
